...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const sectionResult = await client.query(
      ...
      [form_id, title, description || null, display_order]
    )
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE form_section SET title = COALESCE($1, title), description = COALESCE($2, description), display_order = COALESCE($3, display_order) WHERE id = $4 RETURNING *',
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await pool.query('DELETE FROM form_section WHERE id = $1', [req.params.id])
