...
function FieldRenderer({ field, value, onChange }) {
	...
	switch (field.field_type) {
		...
		case 'phone':
			return (
				<input
					...
					placeholder={field.field_type === 'phone' ? 'Enter 10 digit mobile number' : field.field_config?.placeholder || ''}
					className={baseClass}
		...
		case 'number':
			return <input type='number' min='0' step='any' value={value || ''} onChange={(e) => onChange(Number(e.target.value))} className={baseClass} required={field.is_required} />
				...
				<div className='space-y-2'>
					{field.options?.map((opt) => (
						<label key={opt.id} className='flex items-center gap-2'>
							<input type='radio' name={field.field_key} value={opt.value} checked={value === opt.value} onChange={(e) => onChange(e.target.value)} />
		...
		case 'rating':
			return (
				<div className='flex gap-2'>
					{[1, 2, 3, 4, 5].map((n) => (
						<button key={n} type='button' onClick={() => onChange(String(n))} className={`w-10 h-10 rounded-full border-2 ${parseInt(value) === n ? 'bg-yellow-400 border-yellow-500' : 'border-gray-300 hover:border-yellow-400'} flex items-center justify-center font-bold`}>
							{n}
						</button>
					))}
				</div>
			...
			return (
				<div className='overflow-x-auto'>
					<table className='min-w-full border border-gray-300'>
						<thead>
							<tr>
								<th className='border p-2 bg-gray-50'>Question</th>
							...
							{field.matrix_config?.rows?.map((row, rowIdx) => (
								<tr key={rowIdx}>
									...
									{field.matrix_config?.columns?.map((col, colIdx) => {
										...
										const cellValue = isOldFormat ? value?.[row] : value?.[row]?.[colLabel]
										...
										return (
											<td key={colIdx} className='border p-2 text-center'>
												{colType === 'checkbox' ? <input type='checkbox' checked={!!cellValue} onChange={(e) => handleChange(e.target.checked)} /> : colType === 'number' ? <input type='number' min='0' step='any' value={cellValue ?? ''} onChange={(e) => handleChange(Number(e.target.value))} className='w-20 px-2 py-1 border rounded text-sm' /> : colType === 'radio' ? <input type='radio' name={`${field.field_key}_${row}`} checked={cellValue === colLabel} onChange={() => handleChange(colLabel)} /> : <input type='text' value={cellValue ?? ''} onChange={(e) => handleChange(e.target.value)} className='w-full px-2 py-1 border rounded text-sm' />}
											</td>
										)
									})}
								</tr>
		...
		case 'repeatable_group':
			const repeatableFields = field.children || field.field_config?.fields || []
							...
							<div className='space-y-4'>
								{repeatableFields.map((subField) => (
									<div key={subField.field_key}>
										...
										<FieldRenderer
											field={subField}
											value={groupItem[subField.field_key]}
											onChange={(newValue) => {
												const updated = [...(value || [])]
	...
	const validateForm = () => {
		for (const section of form.sections) {
			for (const field of section.fields) {
				...
				// PHONE VALIDATION
				if (field.field_type === 'phone' && value && !/^[0-9]{10}$/.test(value)) {
					toast.error(`${field.label} must be a valid 10 digit mobile number`)
				...
				if (field.field_type === 'matrix') {
					...
					const isOldFormat = rows.some((row) => matrixAnswers[row] && typeof matrixAnswers[row] === 'string')

					if (isOldFormat) {
						if (!field.is_required) continue

						for (const row of rows) {
							if (!matrixAnswers[row] || String(matrixAnswers[row]).trim() === '') {
								toast.error(`${field.label}: ${row} is required`)
						...
						for (const row of rows) {
							for (const col of columns) {
								...
								if (cellValue === undefined || cellValue === null || cellValue === '') {
									toast.error(`${field.label}: ${row} - ${colLabel} is required`)
				...
				if (field.field_type === 'repeatable_group') {
					...
					if (field.is_required && groups.length === 0) {
						toast.error(`${field.label} requires at least one item`)

						return false
					}

					for (const item of groups) {
						for (const subField of field.field_config?.fields || []) {
							const subValue = item[subField.field_key]

							if (subField.is_required && (subValue === undefined || subValue === null || String(subValue).trim() === '')) {
								toast.error(`${subField.label} is required in ${field.label}`)
